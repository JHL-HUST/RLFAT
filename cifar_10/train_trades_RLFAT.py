...
adv_x_input_o = tf.placeholder(tf.float32, shape=[None, 32, 32, 3])
...
model_adv_o = Model(adv_x_input_o, y_input, mode='train', reuse=True)

loss_ft = tf.reduce_sum(tf.squared_difference(model_adv.pre_softmax, model_adv_o.pre_softmax)) * 1.0 / batch_size

# Setting up the optimizer
loss = model.mean_xent + 6.0 * tf.reduce_mean(kullback_leibler_divergence(model.prob, model_adv.prob)) + 1.0 * loss_ft
...
with tf.Session(config=config) as sess:
  ...
  data_flow = data_input.AugmentedData(raw_data, sess, model)
  ...
  for ii in range(start_step, max_num_training_steps+1):
    ...
    x_batch_adv_s = rbs_transformation(x_batch_adv, 2)
    ...
    adv_dict = {model.x_input: x_batch,
                model.y_input: y_batch,
                model_adv.x_input: x_batch_adv_s,
                adv_x_input_o: x_batch_adv}

    # Output to stdout
    if ii % num_output_steps == 0:
      nat_acc = sess.run(model.accuracy, feed_dict=nat_dict)
      adv_acc = sess.run(model_adv.accuracy, feed_dict=adv_dict)
