...
adv_y_input = tf.placeholder(tf.float32, shape=[None, 100])
model = Model(adv_x_input, adv_y_input, mode='train')

# Setting up the optimizer
update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)

with tf.control_dependencies(update_ops):
    train_step = tf.train.AdamOptimizer(1e-3).minimize(model.mean_xent, global_step=global_step)
